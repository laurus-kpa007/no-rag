...
Docling 파싱 결과를 활용하여 Ollama gemma3:27b로 문서를 분석하고
최적의 계층적 구조를 제안합니다.

비정형 문서(헤딩 없음)도 LLM이 논리적 구조를 생성합니다.
"""
...
from .parser import ParsedDocument, Section, sections_to_text_summary
...
def analyze_document(parsed: ParsedDocument) -> DocumentAnalysis:
    """
    Pass 1: 문서의 현재 구조를 분석합니다.

    Docling의 마크다운 출력을 LLM에 전달하여 정확한 분석을 수행합니다.
    ...
    doc_text = _truncate_text(parsed.markdown_text)
    structure_note = (
        "이 문서는 명확한 헤딩 구조를 갖고 있습니다."
        if parsed.is_structured
        else "이 문서는 헤딩이나 목차 없이 텍스트만 나열되어 있는 비정형 문서입니다. "
             "내용의 논리적 흐름을 파악하여 주제별로 분류하세요."
    )
...
- 비정형 문서인 경우, 텍스트의 내용 흐름을 분석하여 논리적 블록으로 나누세요
- 문서의 언어에 맞춰 응답하세요 (한국어 문서면 한국어로)"""
    ...
    try:
        ...
        return DocumentAnalysis(
            ...
            main_topic=result.get('main_topic', parsed.title),
...
def generate_restructure_plan(
    ...
    Docling이 추출한 풍부한 섹션 정보를 활용하여 LLM에 더 정확한 컨텍스트를 제공합니다.
    ...
    if n <= 5:
        min_major = 3
        depth_guide = "2레벨(대분류/중분류) 구조로 구성하세요."
        detail_guide = (
            f"- 대분류(level 1)는 최소 {min_major}개 이상 만드세요\n"
            f"- 내용이 있는 대분류에는 가능하면 중분류(subsections)를 추가하세요"
        )
    elif n <= 15:
        min_major = max(4, n // 3)
        depth_guide = "반드시 2~3레벨(대분류/중분류/소분류) 구조로 구성하세요."
        detail_guide = (
            f"- 대분류(level 1)는 최소 {min_major}개 이상 만드세요\n"
            f"- 원본 섹션이 2개 이상 배정된 대분류는 반드시 중분류(subsections)로 세분화하세요\n"
            f"- 중분류에 원본 섹션이 3개 이상이면 소분류(level 3)도 고려하세요"
        )
    else:
        min_major = max(5, n // 4)
    ...
    if not parsed.is_structured:
        unstructured_note = """
=== 비정형 문서 안내 ===
이 문서는 원래 헤딩이나 목차가 없는 비정형 문서입니다.
원본 '단락' 섹션들은 임시로 분리한 것이므로, 내용을 분석하여 완전히 새로운 논리적 구조를 만들어야 합니다.
각 단락의 내용을 읽고 주제별로 분류하여 체계적인 목차를 설계하세요.
"""
...
- 문서의 언어에 맞춰 응답하세요"""
    ...
    try:
        ...
        return RestructurePlan(
            title=result.get('title', parsed.title),
            toc=toc_items,
            analysis=analysis,
        )
    except (json.JSONDecodeError, KeyError) as e:
        print(f"[분석] Pass 2 JSON 파싱 실패, 기본 구조 사용: {e}")
        return _create_fallback_plan(parsed, analysis)
...
def _create_fallback_plan(
    ...
    if not toc_items:
        toc_items.append(TocItem(
            level=1,
            title='본문',
            source_sections=list(range(len(parsed.sections))),
        ))
